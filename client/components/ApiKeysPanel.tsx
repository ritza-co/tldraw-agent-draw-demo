...
/**
 * Floating "API keys" panel. Lets a visitor paste their own provider keys, which
 * are stored ONLY in their browser (localStorage) and sent as request headers.
 * There is no shared server-side key, so each visitor pays with their own.
 */
export function ApiKeysPanel() {
	...
	// Load saved keys when the panel opens.
	useEffect(() => {
		if (!open) return
		setValues(
